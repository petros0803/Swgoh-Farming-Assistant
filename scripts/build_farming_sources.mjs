/**
 * Generates the canonical catalog and farming locations for every obtainable
 * character and ship.
 *
...
import { alternateFarmingSources } from '../src/data/alternateFarmingSources.js';
...
const CHARACTER_ICON_DIR = path.join(ROOT, 'public', 'assets', 'characters');
const SHIP_ICON_DIR = path.join(ROOT, 'public', 'assets', 'ships');
...
function findMission(campaigns, identifier) {
  ...
  return mission ? { campaign, map, node, mission } : null;
}

function gameEventLabel(resolved, identifier, strings) {
  const localized = strings[resolved.node?.nameKey]
    ?.split('\n')[0]
    .replace(/\[\/?c(?:\]\[[^\]]+)?\]/gi, '')
    .replace(/\[[^\]]+\]/g, '')
    .trim();
  if (localized && !/^placeholder$/i.test(localized)) return localized;

  return identifier.campaignNodeId
    .replace(/^NODE_EVENT_/, '')
    .replace(/_L\d+$/, '')
    .split('_')
    .filter(Boolean)
...
function collectShardUnitIds(value, ids = new Set()) {
  if (typeof value === 'string') {
    if (value.startsWith('unitshard_')) ids.add(value.slice('unitshard_'.length));
    return ids;
  }
  if (Array.isArray(value)) {
    value.forEach((entry) => collectShardUnitIds(entry, ids));
    return ids;
  }
  if (value && typeof value === 'object') {
    Object.values(value).forEach((entry) => collectShardUnitIds(entry, ids));
  }
  return ids;
}

function serialize(catalog, version) {
  return [
    '// Generated by scripts/build_farming_sources.mjs — do not edit by hand.',
    ...
    '',
    `export const unitCatalog = ${JSON.stringify(catalog, null, 2)};`,
    '',
...
async function buildPortraitIndex() {
  const [characterFiles, shipFiles] = await Promise.all([
    readdir(CHARACTER_ICON_DIR),
    readdir(SHIP_ICON_DIR)
  ]);
  const index = (files) => new Map(
    files
      .filter((file) => /\.(png|webp|jpe?g)$/i.test(file))
      .map((file) => [normalize(path.parse(file).name), file])
  );
  return {
    characters: index(characterFiles),
    ships: index(shipFiles)
  };
}

function portraitFor(unit, name, portraits) {
  const files = unit.combatType === 1 ? portraits.characters : portraits.ships;
  const file = files.get(normalize(name)) ?? files.get(normalize(unit.baseId));
  if (!file) return null;
  return `assets/${unit.combatType === 1 ? 'characters' : 'ships'}/${file}`;
}

function mergeSources(generated, curated) {
  const merged = new Map();
  [...generated, ...curated].forEach((source) => {
    const typeKey = ['event', 'journey'].includes(source.type) ? 'unlock' : source.type;
    const key = `${typeKey}:${source.label}`;
    const existing = merged.get(key);
    if (!existing) {
      merged.set(key, source);
    } else if (existing.offers?.length && !source.offers?.length) {
      merged.set(key, { ...source, ...existing });
    } else {
      merged.set(key, { ...existing, ...source });
    }
  });
  const sources = [...merged.values()];
  const hasNamedUnlock = sources.some(
    (source) =>
      (source.type === 'event' && source.communityListed) ||
      source.type === 'journey'
  );
  return hasNamedUnlock
    ? sources.filter((source) =>
      source.type !== 'event' ||
      !source.gameDataListed ||
      source.label === 'Conquest (latest reward set)'
    )
    : sources;
}
...
async function getWikiLocations(name, isCharacter, storesOnly) {
  ...
  try {
    ...
    const locations = await Promise.all(
      Object.values(json.query?.results ?? {}).map((result) =>
        wikiLocationSource(result, name, isCharacter)
      )
    );
    return locations
...
function battleSourceType(label) {
  if (/^Cantina Battles:/i.test(label)) return 'cantina-node';
  if (/^Fleet Battles:/i.test(label)) return 'fleet-node';
  if (/^(Light|Dark) Side Battles:/i.test(label)) return 'hard-node';
  return null;
}

function battleSourceLabel(label) {
  return label
    .replace(/^Light Side Battles:\s*/i, 'Light Side ')
    .replace(/^Dark Side Battles:\s*/i, 'Dark Side ')
    .replace(/^Cantina Battles:\s*/i, 'Cantina ')
    .replace(/^Fleet Battles:\s*/i, 'Fleet ')
    .replace(/\s*\(Hard\)\s*$/i, '');
}

async function wikiLocationSource(result, unitName, isCharacter) {
  const type = battleSourceType(result.fulltext);
  if (!type) {
    return {
  ...
  const url =
    `${WIKI_API}?action=parse&page=${encodeURIComponent(result.fulltext)}` +
    '&prop=wikitext&format=json';
  const response = await fetch(url);
  const json = response.ok ? await response.json() : {};
  const wikitext = json.parse?.wikitext?.['*'] ?? '';
  const energy = Number(wikitext.match(/\|\s*energy\s*=\s*(\d+)/i)?.[1]) || null;
  const rewardType = isCharacter ? 'hero' : 'ship';
  const quantities = [...wikitext.matchAll(/\{\{Reward\|([^|}]+)\|([^|}]*)\|([^|}]*)/gi)]
    .filter((match) =>
      match[1].toLowerCase() === rewardType && normalize(match[2]) === normalize(unitName)
    )
    .map((match) => Number(match[3].match(/(\d+)\s*$/)?.[1]))
    .filter(Number.isFinite);
  const shardsPerDrop = Math.max(1, ...quantities);

  return {
    type,
    label: battleSourceLabel(result.fulltext),
    energy,
    attemptsPerDay: type === 'cantina-node' ? null : 5,
    shardsPerDrop,
    accelerated: shardsPerDrop > 1,
    url: result.fullurl,
    communityListed: true
  };
}
...
async function main() {
  ...
  const roadmapUnitIds = [...roadmap.matchAll(/\bid:\s*"([^"]+)"/g)].map((match) => match[1]);
  const journeySources = new Map(
    [...roadmap.matchAll(/\bevent:\s*"([^"]+)"[\s\S]*?\breward:\s*\{[^}]*\bid:\s*"([^"]+)"/g)]
      .map((match) => [
        match[2],
        { type: 'journey', label: match[1] }
      ])
  );
  ...
  const [
    ...
    getJson('conquestDefinition.json'),
  ...
  const latestConquest = [...(conquestJson.data ?? conquestJson)]
    .sort((a, b) =>
      Number(b.id?.match(/\d+$/)?.[0] ?? 0) - Number(a.id?.match(/\d+$/)?.[0] ?? 0)
    )[0];
  const currentConquestUnitIds = collectShardUnitIds(latestConquest);
  const obtainableUnitIds = [...units.values()]
    .filter((unit) =>
      [1, 2].includes(unit.combatType) && unit.categoryId?.includes('any_obtainable')
    )
    .map((unit) => unit.baseId);
  const unitIds = [...new Set([...roadmapUnitIds, ...obtainableUnitIds])].sort();
  ...
  for (const unitId of unitIds) {
    ...
    for (const lookup of material.lookupMission ?? []) {
      ...
      if (!campaignType) {
        // A marquee is a release event, not an ongoing farming location.
        if (identifier.campaignId === 'EVENTS' && identifier.campaignMapId !== 'MARQUEE') {
          events.push({
            type: 'event',
            label: gameEventLabel(resolved, identifier, strings),
            gameDataListed: true
          });
        }
        continue;
      ...
      nodes.push({
        ...
        shardsPerDrop: shardReward?.minQuantity || null,
        accelerated: shardReward ? shardReward.minQuantity > 1 : null,
        gameDataListed: true
      });
    }
    ...
    if (currentConquestUnitIds.has(unitId)) {
      sources[unitId] = [
        ...(sources[unitId] ?? []),
        {
          type: 'event',
    ...
    serialize(
      unitIds
        .map((id) => {
          ...
          const name = unit && strings[unit.nameKey];
          if (!unit || !name) return null;
          return {
            id,
            name,
            kind: unit.combatType === 1 ? 'character' : 'ship',
            alignment: { 1: 'neutral', 2: 'light', 3: 'dark' }[unit.forceAlignment] ?? 'neutral',
            icon: portraitFor(unit, name, portraits),
            sources: mergeSources(
              [...(sources[id] ?? []), ...(journeySources.has(id) ? [journeySources.get(id)] : [])],
              alternateFarmingSources[id] ?? []
            )
          };
        })
        .filter(Boolean)
        .sort((a, b) => a.name.localeCompare(b.name)),
      versions.gameVersion ?? versions.version ?? 'unknown'
    ),
    'utf8'
  );
