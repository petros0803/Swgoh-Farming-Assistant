...
/**
 * Searchable image picker shared by catalog-sized selectors.
 * Options use { id, name, meta, image, badge, searchText }.
 */
export default function SearchPicker({
  ...
  const visibleOptions = useMemo(
    () => (normalizedQuery
      ? options.filter((option) =>
        (option.searchText || `${option.name} ${option.meta || ''} ${option.badge || ''}`)
          .toLowerCase()
  ...
  const listId = `${label.toLowerCase().replace(/[^a-z0-9]+/g, '-')}-picker-list`;
  ...
  return (
    <Shell ref={containerRef}>
      {open ? (
        ...
        >
          {selected?.image && <TriggerIcon src={selected.image} alt="" />}
          <TriggerText>
            {selected ? (
              <>
                <TriggerName>{selected.name}</TriggerName>
                {selected.meta && <TriggerMeta>{selected.meta}</TriggerMeta>}
              </>
            ) : (
      ...
      {open && (
        <Panel>
          {visibleOptions.length === 0 ? (
            <Empty>{emptyLabel} “{query}”.</Empty>
          ) : (
            <List id={listId} role="listbox" aria-label={listLabel} ref={listRef}>
              {visibleOptions.map((option, index) => (
                ...
                >
                  {option.image && <OptionIcon src={option.image} alt="" loading="lazy" />}
                  <OptionText>
                    <OptionName>{option.name}</OptionName>
                    {option.meta && <OptionMeta>{option.meta}</OptionMeta>}
                  </OptionText>
